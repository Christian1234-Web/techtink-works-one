...
const CustomHeader = ({ getOptions, getOptionValues, getOptionLabels, setChosenPatient, fetchTransaction, chosenPatient }) => {
  return (
...
const InvoiceList = () => {
  ...
  const fetchTransaction = async (page, id) => {
    try {
      const p = page || 1;
      setLoading(true);
      const url = `transactions?page=${p}&limit=10&patient_id=${chosenPatient}&startDate=${startDate}&endDate=${endDate}&bill_source=&status=${status}`;
