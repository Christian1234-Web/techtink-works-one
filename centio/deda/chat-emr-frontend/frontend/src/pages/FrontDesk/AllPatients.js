	...
	const fetchPatients = useCallback(
		async (page, q) => {
			try {
				...
				const p = page || 1;
				const search = q || '';
				const url = `patient/list?page=${p}&limit=${pageLimit}&q=${search}&startDate=${startDate}&endDate=${endDate}&status=${status}&hmo_id=${hmo}&is_opd=${type}`;
		...
		[dispatch, endDate, hmo, startDate, status, type]
	);
									...
									{patients.map((data, i) => {
										return (
											<tr key={i}>
												...
												<td>
													{!data.is_out_patient ? formatPatientId(data) : 'OPD'}
												</td>
