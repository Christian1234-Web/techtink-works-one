...
const OpticianOptometrist = () => {
    ...
    const makePayment = async () => {
        const user = await storage.getItem(USER_COOKIE);
        const data = { userId: user.id, planId: payPlan.id, category: type.type, id: id.id, type: 'registration' }
        try {
    ...
    const fetchPlanInternTraining = useCallback(async () => {
        const url = `plan`;
        try {
            const rs = await request(url, 'GET', true);
            const plan = rs.result.find(e => e.type === type.type);
            setPayPlan(plan);
        } catch (err) {
            console.log(err);
        }
    ...
    useEffect(() => {
        fetchPlanInternTraining();
        fetchDetailsOfId();
    }, [fetchPlanInternTraining, fetchDetailsOfId])
    return (
