...
function WizardOptician() {
    ...
    const createInternship = async () => {
        ...
        const data = {
            userId: user.id, schoolAttended: user_school_optician, dateOfOrientation: user_date_of_orientation_optician, supervisors: supervisor, status: 'pending',
        ...
        try {
            ...
            console.log(rs);
            if (rs.success === true) {
                history.push(`/optician-dashboard/training/${rs?.id}`);
            }
            // handleSuccess();
            // existPageOptician();
    ...
    const createInternshipAndExit = async () => {
        ...
        const data = {
            userId: user.id, schoolAttended: user_school_optician, dateOfOrientation: user_date_of_orientation_optician,
    ...
    const updateInternship = async () => {
        ...
        try {
            ...
            const url = `trainings/update/${id.id}?senderid=${user?.id}`;

            const rsh = await request(url_hos, 'PATCH', true, dataHos);
            const rss = await request(url_sup, 'PATCH', true, dataSup);
            const rs = await request(url, 'PATCH', true, data);
            const fs = await request(`documents/upload?name=training&id=${id.id}`, 'POST', true, docu);
            setError('Successful!');
            setFiles([]);
            setAllFiles([]);
            setLoading(false);
            handleSuccess();
            history.push('/optician-dashboard');
    ...
    const updateInternshipAndExit = async () => {
        ...
        const data = {
            userId: user.id, schoolAttended: user_school_optician, dateOfOrientation: user_date_of_orientation_optician, supervisors: supervisor, status: 'continue',
            hospitals: hospital
        };
        const dataHos = {
            name: hospital_name, address: hospital_address, email: hospital_email, phone: hospital_phone
                                                ...
                                                <Col lg={6}>
                                                    <div className="mb-3">
                                                        ...
                                                        <Input
                                                            ...
                                                            value={user_school_optician}
                                                            onChange={(e) => setUser_school_optician(e.target.value)}
                                                ...
                                                <Col lg={6}>
                                                    <div className="mb-3">
                                                        ...
                                                        <Flatpickr
                                                            ...
                                                            style={{ background: '#fff' }}
                                                            value={user_date_of_orientation_optician}
                                                            onChange={e => setUser_date_of_orientation_optician(e)}
                                                        />
                                        ...
                                        <div className="d-flex align-items-start gap-3 mt-4">
                                            ...
                                            <button
                                                ...
                                                style={{ background: ' rgb(0, 58, 3)', color: '#fff', borderColor: '1px solid rgb(0,58,3)' }} onClick={() => {
                                                    ...
                                                    if (user_date_of_orientation_optician !== '') {
