...
function PracticeRegTwoOptometrist() {
    ...
    const handleFullReg = async () => {
        ...
        const data = {
            userId: user?.id, meansOfIdentification: id_means, identificationNumber: id_no, academics: academic_formOptometrist, emergencyName: emergency_name, emergencyAddress: emergency_address,
        ...
        try {
            ...
            if (rs.success === true) {
                history.push(`/optometrist-dashboard/oo/optometrist/${rs?.id}`);
            }
            setId_means('');
            setId_no('');
            setEmergency_name('');
    ...
    const updateFullReg = async () => {
        ...
        try {
            setLoading(true);
            const url = `optometrists/update/${id.id}?senderid=${user?.id}`;
            const rs = await request(url, 'PATCH', true, data);
            const fs = await request(`documents/upload?name=optometrist&id=${id.id?.id}`, 'POST', true, docu);
            setLoading(false);
            handleSuccess();
            setId_means(' ');
            setId_no('');
    ...
    const updateAndExitFullReg = async () => {
        ...
        try {
            ...
            const url = `optometrists/update/${id.id}?senderid=${user?.id}`;
            const rs = await request(url, 'PATCH', true, data);
            const fs = await request(`documents/upload?name=optometrist&id=${id?.id}`, 'POST', true, docu);
            console.log(fs);
            setLoading(false);
            handleSuccess();
            setId_means(' ');
    ...
    //     if (OptometristTraining === null || OptometristTraining?.isApprovedByAdmin === false || OptometristTraining?.isApprovedByAdmin === null) {
    //         setOptician_countdown('0d  0h 0m 0s');
    //         return
    //     }
    //     else {
    //         let timeSet = new Date(OptometristTraining?.updatedAt).toUTCString().split(' ');
    ...
    //         }, 1000);
    //     }
    // }, [OptometristTraining, setOptician_countdown, startDate]);
    ...
    const fetchUserDetails = useCallback(async () => {
        if (id.id) {
            try {
                ...
                setOptometrist_approval(rs.data.status);
                setLoading(false);
            } catch (err) {
                setLoading(false);
                console.log(err)
                                        ...
                                        <div className="col-lg-12">
                                            ...
                                            <option value="Yada">Yada</option>
                                                            ...
                                                            <div className="col-xxl-6 col-md-5">
                                                                ...
                                                                {optometrist_approval === 'pending' || optometrist_approval === 'processing' ? <input type="text" value={id_means}
                                                                    className="form-control"
                                                                    id="placeholderInput"
                                                            ...
                                                            <div className="col-xxl-6 col-md-8">
                                                                <div>
                                                                    ...
                                                                    <input type="text" onChange={(e) => setId_no(e.target.value)}
                                                                        value={id.id !== null ? id_no : id_no} readOnly={read_only_optometrist} style={{ background: '#fff' }}
                                                                                    ...
                                                                                    <tbody key={i}>
                                                                                        <tr>
                                                                                            ...
                                                                                            <td>
                                                                                                <div className="hstack gap-3 flex-wrap">
                                                                                                    <div className="hstack gap-3 flex-wrap">
                                                                                                        {optometrist_approval === 'continue' ? <Link className="link-success fs-15"><i onClick={() => editRefereeOptometrist(i)} className="ri-edit-2-line"></i></Link>
                                                                                                            : " "}
                                                                                                        {optometrist_approval === 'continue' ? <Link className="link-danger fs-15"><i
